...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow all trusted app subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*flowstate\.app$/)) {
        return true;
      }
      ...
      try {
        ...
        for (const change of variableEditChanges) {
          ...
          if (result.success) {
            ...
            try {
              execSync(`git -c user.name="visual-edit" -c user.email="support@flowstate.app" add "${result.file}"`);
              execSync(`git -c user.name="visual-edit" -c user.email="support@flowstate.app" commit -m "visual_edit_variable_${timestamp}"`);
            } catch (gitError) {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@flowstate.app" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@flowstate.app" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
